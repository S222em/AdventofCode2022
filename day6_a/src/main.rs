...
fn main() {
    ...
    let chars_before_marker = find_chars_before_marker(&file).unwrap();

    println!("{}", chars_before_marker)
}

/// Finds the amount of chars after with the first marker has passed
fn find_chars_before_marker(buffer: &str) -> Option<usize> {
    let mut i = 0;

    while i < buffer.len() - 4 {
        if contains_only_unique_characters(&buffer[i..(i + 4)]) {
            return Some(i + 4);
        }
