...
fn find_shortest_path(grid: &Grid, start: Position, target: Position) -> Option<usize> {
    ...
    while !queue.is_empty() {
        ...
        for direction in [(1, 0), (-1, 0), (0, 1), (0, -1)] {
            let new_position = move_by(position, direction, grid);

            if new_position.is_none() { continue; }
            let new_position = new_position.unwrap();

            if !can_step_to(position, new_position, grid) || !visited.insert(new_position) { continue; }

            if new_position == target { return Some(path_length + 1); }

            queue.push((new_position, path_length + 1));
        }
    }
