...
impl<'a> Monkey<'a> {
    ...
    pub fn throw(&self, item: usize) -> usize {
        let number = self.test[1].parse::<usize>().unwrap();

        let result = match self.test[0] {
            "divisible" => item % number == 0,
...
fn do_rounds(mut monkeys: Vec<Monkey>) -> Vec<Monkey> {
    // Calculates a common multiple between all test values
    let common_multiple: usize = monkeys.iter().map(|monkey| {
        monkey.test[1].parse::<usize>().unwrap()
    }).product();

    for _ in 0..10_000 {
        monkeys = do_round(monkeys, common_multiple)
    }
...
fn do_round(mut monkeys: Vec<Monkey>, common_multiple: usize) -> Vec<Monkey> {
    for i in 0..monkeys.len() {
        for _ in 0..monkeys[i].items.len() {
            let item = monkeys[i].items.remove(0);
            let item = monkeys[i].inspect(item) % common_multiple;
            let throw_to = monkeys[i].throw(item);
            monkeys[throw_to].items.push(item % 100000000);
        }
    }
